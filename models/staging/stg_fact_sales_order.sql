...
SELECT
  ...
  , is_undersupply_backordered AS is_under_supply_backorder_boolean
...
SELECT
  ...
  , CAST(is_under_supply_backorder_boolean AS BOOLEAN) AS is_under_supply_backorder_boolean
...
, fact_sales_order__convert_boolean AS (
  SELECT
    *
    , CASE 
        WHEN is_under_supply_backorder_boolean = TRUE THEN "Under Supply Backorder"
        WHEN is_under_supply_backorder_boolean = FALSE THEN "Not Under Supply Backorder"
        WHEN is_under_supply_backorder_boolean IS NULL THEN "Undefined"
        ELSE "Invalid"
      END AS is_under_supply_backorder
...
, fact_sales_order__coalesce_record AS (
  SELECT
    sales_order_key
    , COALESCE(is_under_supply_backorder, "Undefined") AS is_under_supply_backorder
...
, fact_sales_order__undefined_record AS (
  ...
  SELECT
    ...
    , "Undefined" AS is_under_supply_backorder
    , 0 AS customer_key
  ...
  SELECT
    ...
    , "Invalid" AS is_under_supply_backorder
    , -1 AS customer_key
